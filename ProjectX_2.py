...
if not final_report.empty:
    ...
    with pd.ExcelWriter(output_file, engine='openpyxl') as writer:
        ...
        try:
            ...
            # Add conditional formatting (green data bars) to Budget Remaining column per project
            from openpyxl.formatting.rule import DataBar, FormatObject, Rule
            from openpyxl.styles import Color
            
            # Find the Budget Remaining column index and Project column index
            try:
                br_idx = list(final_report.columns).index('Budget Remaining')
                br_col_letter = chr(65 + br_idx) if br_idx < 26 else chr(64 + br_idx//26) + chr(65 + br_idx%26)
                
                # Check if Project column exists
                if 'Project' in final_report.columns:
                    project_idx = list(final_report.columns).index('Project')
                    ...
                    for i, row in enumerate(worksheet.iter_rows(min_row=2, max_row=len(final_report)+1, min_col=project_idx+1, max_col=project_idx+1), start=2):
                    ...
                    if current_project is not None:
                        unique_projects.append((current_project, start_row, len(final_report)+1))
                    ...
                    for proj_idx, (project, start_row, end_row) in enumerate(unique_projects):
                        ...
                        print(f"Added conditional formatting for project {project} (rows {start_row}-{end_row})")
                else:
                    # If no Project column exists, apply formatting to the entire column
                    green_databar = DataBar(
                        cfvo=[FormatObject(type='min'), FormatObject(type='max')],
                        color="00B050",  # Green color
                        showValue=True,
                    ...
                    rule = Rule(type='dataBar', dataBar=green_databar)
                    cell_range = f"{br_col_letter}2:{br_col_letter}{len(final_report) + 1}"
                    worksheet.conditional_formatting.add(cell_range, rule)
                    
                    print(f"No Project column found. Added formatting to entire Budget Remaining column.")
            except Exception as e:
                print(f"Warning: Could not apply conditional formatting to Budget Remaining: {e}")
                import traceback
                traceback.print_exc()
