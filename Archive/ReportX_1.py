...
if not final_report.empty:
    ...
    with pd.ExcelWriter(output_file, engine='openpyxl') as writer:
        ...
        try:
            ...
            # Add conditional formatting (green data bars) to Budget Remaining column
            ...
            try:
                ...
                # Apply the conditional formatting to the Budget Remaining column
                rule = Rule(type='dataBar', dataBar=green_databar)
                cell_range = f"{br_col_letter}2:{br_col_letter}{len(final_report) + 1}"
                worksheet.conditional_formatting.add(cell_range, rule)
                
                print(f"Added green data bar conditional formatting to Budget Remaining column ({br_col_letter})")
            except Exception as e:
