"""
This script processes financial data from multiple files (AE, PT, and P files) in various formats (CSV, Excel, TXT) 
to generate a consolidated financial report. The script performs the following tasks:
1. Searches for files containing "AE" (Activity Estimates), "PT" (Project Transactions), and "P" (Project) in the 
   specified folder.
2. Reads and concatenates data from these files, handling different file formats and encodings.
3. Normalizes and matches column names to extract required fields, handling variations in column naming.
4. Extracts project manager information from P files and maps it to projects.
5. Aggregates actual costs from PT data by Activity Sequence.
6. Merges AE and PT data to create a final report, calculating budget remaining (Estimated Cost - Actual Cost).
7. Adds manager descriptions to the report and sorts it by Project and Budget Remaining.
8. Creates an Excel report with:
   - A main 'Activity Report' sheet containing all data.
   - Separate sheets for each manager's projects.
   - An 'Employee Hours' sheet with Internal Quantity, Report Code Description, Project Description, 
     Project Activity Sequence, and Employee Description.
   - Formatting including bold headers, auto-adjusted column widths, currency formats for cost/revenue columns, 
     and data bars for Budget Remaining by project (except in Employee Hours tab).
9. Saves the report as 'reportX.xlsx' in the input folder. If the file is open, appends the current date (DDMMYY) 
   to the filename (e.g., 'reportX_160525.xlsx') to avoid errors.
10. Handles errors, missing columns, and empty datasets gracefully, providing detailed logging throughout the process.
"""
...
if not final_report.empty or not employee_hours.empty:
    # Define the base output file
    base_output_file = os.path.join(folder_path, "reportX.xlsx")
    
    # Get current date for fallback filename (format DDMMYY)
    current_date = datetime.now().strftime("%d%m%y")
    fallback_output_file = os.path.join(folder_path, f"reportX_{current_date}.xlsx")
    
    # Try writing to the base file first
    ...
    try:
        ...
        # Attempt to write to the base file
        ...
        # If the base file is open, use the fallback file with date suffix
        output_file = fallback_output_file
        print(f"Warning: Could not write to {base_output_file}. Using fallback file: {output_file}")
        ...
        print("Please check if the output file is open or if there are other issues.")
else:
