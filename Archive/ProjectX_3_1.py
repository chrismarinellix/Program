...
if not final_report.empty or not employee_hours.empty:
    ...
    with pd.ExcelWriter(output_file, engine='openpyxl') as writer:
        ...
        # Create separate tabs for each project manager
        if not final_report.empty and 'Manager Description' in final_report.columns:
            # Get unique managers, excluding NaN
            managers = [m for m in final_report['Manager Description'].unique() if pd.notna(m)]
            if not managers:
                print("No valid project managers found for separate tabs")
            else:
                print(f"Creating separate tabs for {len(managers)} project managers")
                
                # Track used sheet names to avoid duplicates
                used_sheet_names = set(['Activity Report', 'Employee Hours'])
                
                for manager in managers:
                    # Clean and create a unique sheet name
                    base_name = str(manager)[:28]  # Reserve space for disambiguation
                    ...
                    if not manager_data.empty:
                        ...
                        print(f"  - Skipped tab for manager: {manager} (no data)")
